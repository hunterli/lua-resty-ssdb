...
=== TEST 1: github issue #108: ngx.locaiton.capture + ssdb.set_keepalive
--- http_config eval: $::HttpConfig
--- config
    location /r1 {
        default_type text/html;
        set $ssdb_host $TEST_NGINX_SSDB_HOST;
        set $ssdb_port $TEST_NGINX_SSDB_PORT;
        #lua_code_cache off;
...
local db = ssdb:new()
local ok, err = db:connect(ngx.var.ssdb_host, ngx.var.ssdb_port)
if not ok then
...
--- config
    ...
    location /foo {
        access_by_lua '
            ...
            -- by a ssdb server:
            --     local ok, err = db:connect("unix:/path/to/ssdb.sock")
